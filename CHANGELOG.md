...
- feat(ai): ajouter la dictée vocale dans la fenêtre Poser une question
...
- fix(simulation): remplacer le solde final supposé par un retour à l'équilibre
...
- feat(transactions): export the current ledger view to CSV
...
- IA locale : mode IA sur la version web (serveur llama.cpp externe)
...
- fix(nl-query): answer opérations récurrentes from the bill schedule, not the ledger
...
- feat(update): check for new version before the PIN prompt
