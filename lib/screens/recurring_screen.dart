...
class _RecurringScreenState extends State<RecurringScreen> {
  ...
  // Tracks the last DatabaseProvider.selectedAccountId this screen synced
  // _accountFilter from - see the sync block at the top of build() for why
  // this can't just be seeded once in initState(). _hasSyncedAccountFilter
  // exists only so the very first sync still runs even when
  // selectedAccountId itself happens to be null (a bare
  // "!= _syncedSelectedAccountId" check would silently skip it, since both
  // start out null).
  int? _syncedSelectedAccountId;
  bool _hasSyncedAccountFilter = false;

  @override
  Widget build(BuildContext context) {
    final dbProvider = context.watch<DatabaseProvider>();
    final repo = dbProvider.repository!;
    // Par défaut (et à chaque fois que le compte sélectionné change ailleurs
    // dans l'app - Tableau de bord ou Grand livre), aligné sur ce même
    // compte (2026-09 user request: "ce compte doit être le même si on
    // passe sur la page des opérations récurrentes") - re-synced here
    // rather than once in initState(), puisque HomeShell garde cet écran
    // monté en permanence dans un IndexedStack pour la barre de navigation
    // du bas (jamais recréé en changeant d'onglet) ; sans ça, initState() ne
    // s'exécuterait qu'une seule fois pour toute la session et ce filtre
    // resterait figé sur le tout premier compte sélectionné. Reste
    // modifiable ensuite via le menu "Filtrer par compte", qui ne redéclenche
    // pas ce bloc puisqu'il ne touche pas selectedAccountId.
    if (!_hasSyncedAccountFilter ||
        dbProvider.selectedAccountId != _syncedSelectedAccountId) {
      _hasSyncedAccountFilter = true;
      _syncedSelectedAccountId = dbProvider.selectedAccountId;
      _accountFilter = dbProvider.selectedAccountId;
    }
  ...
  Future<void> _recordOccurrence(BuildContext context, BillDeposit bill) async {
    ...
    await showDialog(
      context: context,
      builder: (_) => RecordOccurrenceDialog(bill: bill, repo: repo),
    );
    dbProvider.touch();
  }
